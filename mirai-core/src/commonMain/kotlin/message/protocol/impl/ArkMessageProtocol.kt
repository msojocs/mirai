...
import net.mamoe.mirai.internal.network.protocol.packet.chat.ArkPacket
import net.mamoe.mirai.message.data.*

internal class ArkMessageProtocol : MessageProtocol() {
...
//        add(Decoder())
        ...
        MessageSerializer.superclassesScope(MessageContent::class, SingleMessage::class) {
            add(MessageSerializer(ArkAppMessage::class, ArkAppMessage.serializer(), registerAlsoContextual = true))
        }
    }

    private class Encoder : MessageEncoder<ArkAppMessage> {
        override suspend fun MessageEncoderContext.process(data: ArkAppMessage) {
            markAsConsumed()
    ...
    open class Sender : OutgoingMessageSender {
        override suspend fun OutgoingMessagePipelineContext.process() {
            val arkAppMessage = currentMessageChain[ArkAppMessage] ?: return
            markAsConsumed()
            ...
            sendMusicSharePacket(bot, arkAppMessage, contact, strategy)
        ...
        protected open suspend fun sendMusicSharePacket(
            ...
            val packet = ArkPacket(
                bot.client, contact.id, arkAppMessage,
            ...
            result.pkg.checkSuccess("send ark message")
        }
    }
}